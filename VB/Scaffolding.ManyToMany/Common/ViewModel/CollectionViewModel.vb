...
Namespace Scaffolding.ManyToMany.Common.ViewModel
    ...
    Public MustInherit Class CollectionViewModelBase(Of TEntity As Class, TPrimaryKey, TUnitOfWork As IUnitOfWork)
        ...
        Private ReadOnly newEntityInitializerFactory As Func(Of IEntityInitializer(Of TEntity, TUnitOfWork))
        ...
        Protected Sub New(ByVal unitOfWorkFactory As IUnitOfWorkFactory(Of TUnitOfWork), ByVal getRepositoryFunc As Func(Of TUnitOfWork, IRepository(Of TEntity, TPrimaryKey)), ByVal newEntityInitializerFactory As Func(Of IEntityInitializer(Of TEntity, TUnitOfWork)))
            ...
            Me.newEntityInitializerFactory = newEntityInitializerFactory
        End Sub

        Protected Shadows ReadOnly Property Repository() As IRepository(Of TEntity, TPrimaryKey)
            Get
                Return DirectCast(MyBase.Repository, IRepository(Of TEntity, TPrimaryKey))
            End Get
        End Property
        ...
        Public Overridable Sub [New]()
            Dim document As IDocument = CreateDocument(If(newEntityInitializerFactory IsNot Nothing, newEntityInitializerFactory(), Nothing))
        ...
        Protected Function FindNewEntity(ByVal key As TPrimaryKey) As TEntity
            ...
            Dim entities_Renamed = GetFilteredQueryableEntities()
            Return entities_Renamed.Where(ExpressionHelper.GetValueEqualsExpression(Repository.GetPrimaryKeyExpression, key)).FirstOrDefault()
        End Function
