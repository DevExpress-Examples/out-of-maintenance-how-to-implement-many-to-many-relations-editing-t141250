...
Namespace Scaffolding.ManyToMany.Common.ViewModel
    ...
    Public MustInherit Class EntityPropertyInitializerBase(Of TEntity As Class, TUnitOfWork As IUnitOfWork, TProperty)
        ...
        Private ReadOnly setPropertyAction As Action(Of TEntity, TProperty)
        ...
        Public Sub New(ByVal setPropertyAction As Action(Of TEntity, TProperty))
            Me.setPropertyAction = setPropertyAction
        End Sub

        Protected MustOverride Function TryGetPropertyValue(ByVal unitOfWork As TUnitOfWork, ByVal entity As TEntity, <System.Runtime.InteropServices.Out()> ByRef propertyValue As TProperty) As Boolean

        Private Sub IEntityInitializerGeneric_InitializeEntity(ByVal unitOfWork As TUnitOfWork, ByVal entity As TEntity) Implements IEntityInitializer(Of TEntity, TUnitOfWork).InitializeEntity
            Dim propertyValue As TProperty = Nothing
    ...
    Public Class EntityNavigationPropertyInitializer(Of TEntity As Class, TRelatedEntity As Class, TRelatedEntityKey, TUnitOfWork As IUnitOfWork)
        ...
        Private ReadOnly relatedEntityKey As TRelatedEntityKey
        Private ReadOnly getRepositoryFunc As Func(Of TUnitOfWork, IRepository(Of TRelatedEntity, TRelatedEntityKey))
        ...
        Public Sub New(ByVal relatedEntityKey As TRelatedEntityKey, ByVal setRelatedEntityAction As Action(Of TEntity, TRelatedEntity), ByVal getRepositoryFunc As Func(Of TUnitOfWork, IRepository(Of TRelatedEntity, TRelatedEntityKey)))
            ...
            Me.relatedEntityKey = relatedEntityKey
            Me.getRepositoryFunc = getRepositoryFunc
        End Sub

        Protected Overrides Function TryGetPropertyValue(ByVal unitOfWork As TUnitOfWork, ByVal entity As TEntity, <System.Runtime.InteropServices.Out()> ByRef propertyValue As TRelatedEntity) As Boolean
            Dim relatedEntityRepository As IRepository(Of TRelatedEntity, TRelatedEntityKey) = getRepositoryFunc(unitOfWork)
