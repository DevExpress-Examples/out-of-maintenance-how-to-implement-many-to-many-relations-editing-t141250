...
Namespace Scaffolding.ManyToMany.Common.ViewModel
    Public Class AddRemoveDetailEntitiesViewModel(Of TEntity As Class, TPrimaryKey, TDetailEntity As Class, TDetailPrimaryKey, TUnitOfWork As IUnitOfWork)
        ...
        Protected Overrides Sub OnInitializeInRuntime()
            ...
            Messenger.Default.Register(Of EntityMessage(Of TEntity, TPrimaryKey))(Me, AddressOf OnMessage)
        End Sub
        ...
        Public Sub AddDetailEntities()
            ...
            If DialogService.ShowDialog(MessageButton.OKCancel, "Select objects to add", selectEntitiesViewModel) = MessageResult.OK AndAlso selectEntitiesViewModel.SelectedEntities.Any() Then
        ...
        Private Sub SaveChangesAndNotify(ByVal detailEntities As IEnumerable(Of TDetailEntity))
            Try
                ...
                For Each entity_Renamed In detailEntities
                    Messenger.Default.Send(New EntityMessage(Of TDetailEntity, TDetailPrimaryKey)(DetailsRepository.GetPrimaryKey(entity_Renamed), EntityMessageType.Changed))
                Next entity_Renamed
                ReloadDetails()
